    ...
    (0..(length << 1))
        .map(|c| {
            if c & 1 == 0 {
                ...
                -(((c - 1) as f32) * (std::f32::consts::PI * 2.0) / length_f32)
                    .sin()
            }
        })
        .collect::<Vec<f32>>()
...
/// Merge 2 process into one
/// for block size of 2 and 4
#[inline]
fn radx4_merge_2_4(outslice: &mut [f32]) {
    outslice.chunks_mut(8).for_each(|out_slice| {
        ...
        // Angle diff: PI
        // Expression: first + w * second, first - w * second;
        // w *= w_delta; (w_delta: w(PI) = (-1, 0))
        let (r1, i1, r2, i2, r3, i3, r4, i4) = (
            c1r + c2r,
            c1i + c2i,
            c1r - c2r,
            c1i - c2i,
...
/// for block size of 2 and 4
#[inline]
fn radx4i_merge_2_4(outslice: &mut [f32]) {
    outslice.chunks_mut(8).for_each(|out_slice| {
        let (c1r, c1i, c2r, c2i, c3r, c3i, c4r, c4i) = (
            out_slice[0],
            out_slice[1],
...
fn radx4fft_merge_n_2n(out: &mut [f32], lookup_table: &[f32], block_size: usize, length_lookup_incr: usize) {
    ...
    out.chunks_mut(block_jump_radx4).for_each(|out_chunk| {
        ...
        for index in (0..block_size).step_by(2) {
            ...
            (w2r, w2i) = (lookup_table[lookup_index_radx4], -lookup_table[lookup_index_radx4 + 1]);
            (w3r, w3i) = (lookup_table[lookup_index_radx4 + mid], -lookup_table[lookup_index_radx4 + mid + 1]);
        }
    });
}

fn radx4fft_merge_n(out: &mut [f32], lookup_table: &[f32], block_size: usize, length_check_lookup: usize) {
...
/// Generalize process of radix-4 merge
/// for block size of n and 2*n where n -> 2^k, k \in Natural numbers
fn radx4ifft_merge_n_2n(out: &mut [f32], lookup_table: &[f32], block_size: usize, length_lookup_incr: usize) {
    // Chunks of block_size
    let block_jump = block_size << 1;
    let block_jump_radx4 = block_jump << 1;
    let lookup_incr = length_lookup_incr << 1;
...
fn radx4ifft_merge_n(out: &mut [f32], lookup_table: &[f32], block_size: usize, length_check_lookup: usize) {
    let lookup_incr = length_check_lookup << 1;
    let block_jump = block_size << 1;
...
pub fn radx4fft(array: &[f32], lookup_table: &[f32]) -> Vec<f32> {
    ...
    while block_size <= length {
        if (block_size << 1) <= length {
            radx4fft_merge_n_2n(&mut out, lookup_table, block_size, length_check_lookup);
        } else {
...
pub fn radx4ifft(array: &[f32], lookup_table: &[f32]) -> Vec<f32> {
    ...
    while block_size <= length {
        if (block_size << 1) <= length {
            radx4ifft_merge_n_2n(&mut out, lookup_table, block_size, length_check_lookup);
        } else {
            radx4ifft_merge_n(&mut out, lookup_table, block_size, length_check_lookup);
        }
        block_size <<= 2;
        length_check_lookup >>= 2;
    }
